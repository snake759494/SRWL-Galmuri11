...
> ### 최신판 — v1.2
> [**릴리즈에서 `SRWL_K_v0.9.1_Galmuri11_IMG.xdelta` 를 받으세요**](https://github.com/snake759494/SRWL-Galmuri11/releases/latest)
> (폰트 + 이미지 통합 패치). 적용 방법은 아래 "적용 방법" 또는 [BUILD.md](BUILD.md) 참고.
>
> *이 저장소는 이전 계정(snake7594, 정지됨)에 있던 같은 프로젝트를 이어받은 것입니다.
> 커밋 기록은 그대로 옮겨왔고, 옛 계정의 릴리즈 파일은 남아 있지 않으므로 이 저장소의 릴리즈를 이용하세요.*

- **v1.0** — 대사 폰트를 [갈무리11(Galmuri11)](https://github.com/quiple/galmuri) 비트맵 폰트로 교체 (한글 2,350자)
- **v1.2 (최신, 권장)** — v1.0의 폰트 교체 + 그래픽(이미지)으로 그려진 일본어 텍스트 약 170개 화면 한글화. 이 패치 하나로 폰트·이미지가 모두 적용됩니다. (v1.1의 이미지 한글화를 실기 테스트로 다듬은 안정판)
...
3. 패치 파일을 선택해 적용합니다.
   - **v1.2 (폰트+이미지, 권장)**: `patch/SRWL_K_v0.9.1_Galmuri11_IMG.xdelta` → 결과물 MD5: `A6FF7A389EF7AA1E4DB146E59755E8C1`
...
- **전투 표시 65장** — 베어내기·쏘아떨구기·각종 배리어명·스탯 다운 L1~3·MAP병기·합체 공격 등
...
- **지형적응 문자(空陸海宇)** — 요청에 따라 원본 유지
- 엔딩 감사 화면 1장은 차기 버전 예정

## 버전 이력

- **v1.2** — v1.1 실기 테스트 후 다듬음: 정신기 글자 얇게·외곽선 정리, 크리티컬 배너 재작업(원본 스타일·잔여 조각 제거), 지역명 색 수정, 오프닝 양끝 여백(v1.1에서 「팽창…알지」 줄의 양끝 글자가 잘려 보이던 문제 — kevinjo1111 님 제보, 감사합니다), 세이브 화면 원복(깨짐 수정), 지형문자 원복, 원 제작팀(슈코넷) 크레딧·라이선스 명기
- **v1.1** — 이미지 한글화 최초 공개
- **v1.0** — 갈무리11 폰트 교체

## 버그·오타 제보

플레이 중 발견한 화면 깨짐·잘림·오역은 **[Issues](https://github.com/snake759494/SRWL-Galmuri11/issues)** 에 남겨 주세요.
**패치 버전 · 에뮬레이터/기기 · 발생 위치(몇 화, 어느 화면) · 스크린샷**을 적어 주시면 빠르게 확인합니다.
(원본/패치 ROM 파일 `*.nds`는 저작권상 첨부하지 마세요.)
